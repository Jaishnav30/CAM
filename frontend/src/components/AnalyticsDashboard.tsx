      ...
      <div className="card" style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
        ...
        {dailyChartMetrics.days.length === 0 ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
            ...
            {/* Daily Chronological Ledger Table */}
            <div className="table-responsive">
              <table className="data-table">
                ...
                <tbody>
                  {dailyChartMetrics.days.map((t) => (
                    <tr key={t.dateStr}>
                      <td style={{ fontWeight: 600, fontFamily: 'monospace' }}>
                        {t.dateStr}
                      </td>
                      <td style={{ color: 'var(--color-income)', fontWeight: 600 }}>
                        {t.inVal > 0 ? `+${formatCurrency(t.inVal)}` : '₹0.00'}
                      </td>
                      <td style={{ color: 'var(--color-expense)', fontWeight: 600 }}>
                        {t.outVal > 0 ? `-${formatCurrency(t.outVal)}` : '₹0.00'}
                      </td>
                      <td
                        style={{
                          fontWeight: 700,
                          color: t.netVal >= 0 ? 'var(--color-income)' : 'var(--color-expense)',
                        }}
                      >
                        {formatCurrency(t.netVal)}
                      </td>
